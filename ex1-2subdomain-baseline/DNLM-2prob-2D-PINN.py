...
## ----------------------------- ##
## parser arguments
parser = argparse.ArgumentParser(description='DN-PINNs method for Poisson problem(ex1)')

# path for saving results
parser.add_argument('-r', '--result', default='Results/1_2Prob-2Dcos/DN-PINNs/G1e_2-N2e4-baseline/simulation-test', type=str, metavar='PATH', help='path to save checkpoint')

# optimization options
parser.add_argument('--num_epochs', default=3000, type=int, metavar='N', help='number of total epochs to run')
parser.add_argument('--beta', default=400, type=int, metavar='N', help='penalty coefficeint for mismatching of boundary data')
parser.add_argument('--milestones', type=int, nargs='+', default=[1000, 2000], help='decrease learning rate at these epochs')
...
while((ite_index < args.max_ite_num)):
    ...
    g_right =  torch.autograd.grad(outputs=g_right_temp, inputs=SmpPts_Intfc, grad_outputs=torch.ones_like(g_right_temp), retain_graph=True, create_graph=True, only_inputs=True)[0][:,0]        
    g_right = - g_right.detach().reshape(-1,1)
    # right subproblem-solving
    args.num_epochs = 1000
    model_right, error_L2_right, error_H1_right = NeumannSolverPINN(args, traindata_bndry_G, dataloader_bndry_G, SmpPts_Intfc, g_right, ite_index, 2)
