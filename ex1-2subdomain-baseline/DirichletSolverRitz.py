...
def DirichletSolverRitz(args, traindata_bndry_G, dataloader_bndry_G, SmpPts_Intfc, g_1, iter_num, sub_dom=1):
    ...
    class TraindataGamma(Dataset):    
        def __init__(self, num_bndry_pts_G, dim_prob):         
            ...
            self.h_SmpPts = Exact_Solution.h_Exact_Square2D(self.SmpPts_Bndry_G[:,0], self.SmpPts_Bndry_G[:,1], alpha)        
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            loss_intrr = torch.mean(0.5 * torch.sum(torch.pow(grad_u_NN_intrr, 2), dim=1) - ft_smppts * torch.squeeze(u_NN_intrr))
            loss_bndry_D = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_D) - g_smppts, 2))
            loss_bndry_G = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_G) - g_1_select.reshape(1,-1), 2))
