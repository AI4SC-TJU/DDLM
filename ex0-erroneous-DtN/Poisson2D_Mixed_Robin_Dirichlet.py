...
import argparse
from Poisson2D_Mixed_Robin_Dirichlet_model import Sampler, Poisson2D
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--result', default='Results/Poisson/Square2D/Robin/simulation-test-M1/', type=str, metavar='PATH', help='path to save checkpoint')
    ...
    parser.add_argument('--alpha', type=float, default=1, help='alpha of the inner subproblem')    
    ...
    bc2 = Sampler(2, bc2_coords, lambda x: u(x, a_1, a_2) + args.alpha * u_x(x, a_1, a_2), name='Dirichlet BC2')
    ...
    x1_rel = np.linspace(0.3, 0.5, nn)[:, None]
    x2_rel = np.linspace(0.2, 0.8, nn)[:, None]
    x1_rel, x2_rel = np.meshgrid(x1_rel, x2_rel)
    X_star_rel = np.hstack((x1_rel.flatten()[:, None], x2_rel.flatten()[:, None]))
    ...
    u_star_rel = u(X_star_rel, a_1, a_2)
    f_star_rel = f(X_star_rel, a_1, a_2, k)
    grad_u_star_x_rel = u_x(X_star_rel, a_1, a_2)
    grad_u_star_y_rel = u_y(X_star_rel, a_1, a_2)  
    # Predictions
    u_pred = model.predict_u(X_star)
    f_pred = model.predict_r(X_star)
    grad_u_pred = model.predict_grad_u(X_star)

    u_pred_rel = model.predict_u(X_star_rel)
    f_pred_rel = model.predict_r(X_star_rel)
    grad_u_pred_rel = model.predict_grad_u(X_star_rel)
    # Relative error
    ...
    U_star_rel = griddata(X_star_rel, u_star_rel.flatten(), (x1_rel, x2_rel), method='cubic')
    F_star_rel = griddata(X_star_rel, f_star_rel.flatten(), (x1_rel, x2_rel), method='cubic')
    grad_U_star_x_rel = griddata(X_star_rel, grad_u_star_x_rel.flatten(), (x1_rel, x2_rel), method='cubic')
    grad_U_star_y_rel = griddata(X_star_rel, grad_u_star_y_rel.flatten(), (x1_rel, x2_rel), method='cubic')
    grad_U_star_rel = np.concatenate((grad_U_star_x_rel.reshape(-1,1), grad_U_star_y_rel.reshape(-1,1)), axis=1)
    ...
    U_pred_rel = griddata(X_star_rel, u_pred_rel.flatten(), (x1_rel, x2_rel), method='cubic')
    F_pred_rel = griddata(X_star_rel, f_pred_rel.flatten(), (x1_rel, x2_rel), method='cubic')
    grad_U_pred_x_rel = griddata(X_star_rel, grad_u_pred_rel[:,0].reshape(-1,1).flatten(), (x1_rel, x2_rel), method='cubic')
    grad_U_pred_y_rel = griddata(X_star_rel, grad_u_pred_rel[:,1].reshape(-1,1).flatten(), (x1_rel, x2_rel), method='cubic')
    grad_U_pred_rel = np.concatenate((grad_U_pred_x_rel.reshape(-1,1), grad_U_pred_y_rel.reshape(-1,1)), axis = 1)
    ...
    loss_bcs_robin = model.loss_bcs_robin_log
    fig_2 = plt.figure(2)
    ax = fig_2.add_subplot(1, 1, 1)
    ax.plot(loss_res, label='$\mathcal{L}_{r}$')
    ax.plot(loss_bcs, label='$\mathcal{L}_{u_b}$')
    ax.plot(loss_bcs_robin, label='$\mathcal{L}_{robin}$')
    ax.set_yscale('log')
    ...
    adaptive_constant_robin = model.adaptive_constant_robin_log
    fig_3 = plt.figure(3)
    ax = fig_3.add_subplot(1, 1, 1)
    ax.plot(adaptive_constant, label='$\lambda_{u_b}$')
    ax.plot(adaptive_constant_robin, label='$\lambda_{u_{robin}}$')
    ax.set_xlabel('iterations')
    plt.legend()
    plt.tight_layout()
    ...
    io.savemat(args.result+"/u_exact_rel.mat",{"u_exact_rel": U_star_rel.reshape(-1,1)})
    io.savemat(args.result+"/u_NN_ite%d_test_rel.mat"%ite_index,{"u_NN_test_rel": U_pred_rel.reshape(-1,1)})
    io.savemat(args.result+"/gradu_exact_rel.mat",{"grad_u_exact_rel": grad_U_star_rel.reshape(-1,2)})    
    io.savemat(args.result+"/gradu_NN_ite%d_test_rel.mat"%ite_index,{"grad_u_NN_test_rel": grad_U_pred_rel.reshape(-1,2)})
