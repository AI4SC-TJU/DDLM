...
def NeumannSolverPINN(args, traindata_bndry_G, dataloader_bndry_G, SmpPts_Intfc, g_1, iter_num, sub_dom=1):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            #smppts_bndry_D.requires_grad = True
            ...
            model.zero_grad()
            grad_u_NN_interior = torch.autograd.grad(outputs=u_NN_interior, inputs=smppts_intrr, grad_outputs=torch.ones_like(u_NN_interior), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            ...
            loss_intrr_epoch += loss_interior.item() * smppts_intrr.size(0) / traindata_intrr.SmpPts_Interior.shape[0]
