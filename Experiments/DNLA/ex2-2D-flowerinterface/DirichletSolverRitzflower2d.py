...
def DirichletSolverRitzflower2d(args, traindata_bndry_G, dataloader_bndry_G, SmpPts_Intfc, iter_num, sub_dom=1):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            u_NN_intrr_ex = Exact_Solution_radius.u_Exact(smppts_intrr, args.alpha_R, args.alpha_B, sub_dom)
            ...
            grad_u_NN_intrr_ex = Exact_Solution_radius.Grad_u_Exact(smppts_intrr,args.alpha_R, args.alpha_B, sub_dom)
    ...
    #         loss_bndry_N = torch.mean(torch.pow(- grad_u_NN_bndry_N[:,0] - g_N_smppts, 2))
            ...
            if sub_dom == 1:
                loss_intrr = torch.mean(0.5 * args.alpha_R * torch.sum(torch.pow(grad_u_NN_intrr, 2), 1) - torch.squeeze(f_smppts) * torch.squeeze(u_NN_intrr))
                loss_intrr_ex = torch.mean(0.5 * args.alpha_R * torch.sum(torch.pow(grad_u_NN_intrr_ex, 2), 1) - torch.squeeze(f_smppts) * torch.squeeze(u_NN_intrr_ex))
            if sub_dom == 2:
                loss_intrr = torch.mean(0.5 * args.alpha_B * torch.sum(torch.pow(grad_u_NN_intrr, 2), 1) - torch.squeeze(f_smppts) * torch.squeeze(u_NN_intrr))
