...
def NeumannSolverPINNflower2d(args, traindata_bndry_G, dataloader_bndry_G, model_in, iter_num, sub_dom=1):
    ...
    ##############################################################################################
    plt.figure()
    plt.scatter(traindata_intrr.SmpPts_Interior[:,0], traindata_intrr.SmpPts_Interior[:,1], c = 'red' , label = 'interior points sub_dom2' )
    plt.scatter(traindata_bndry_D.SmpPts_Bndry_D[:,0],  traindata_bndry_D.SmpPts_Bndry_D[:,1], c = 'black', label = 'boundary of sub_dom2' )
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            grad_u_R = torch.autograd.grad(outputs=u_NN_bndry_R, inputs=smppts_bndry_G, grad_outputs=torch.ones_like(u_NN_bndry_R), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            
            
            u_in_R = model_in(smppts_bndry_G.float())
            grad_u_in_R = torch.autograd.grad(outputs=u_in_R, inputs=smppts_bndry_G, grad_outputs=torch.ones_like(u_in_R), retain_graph=True, create_graph=True, only_inputs=True)[0].detach()       
            arclen = Sample_Points_quarter.arclength(len(smppts_bndry_G), dim_prob).to(device)             
            #grad_u_R_ex = Exact_Solution_radius.Grad_u_Exact(smppts_bndry_G.float(), args.alpha_R, args.alpha_B, 1)
            vectorn = normalvector(smppts_bndry_G, 1)
            ...
            loss_intrr = torch.mean((-args.alpha_B * torch.squeeze(laplace_u) - torch.squeeze(f_smppts))**2)
    ...
    #         loss_bndry_N = torch.mean(torch.pow(- grad_u_NN_bndry_N[:,0] - g_N_smppts, 2))
            ...
            loss_bndry_R = torch.mean(torch.pow(torch.sum(args.alpha_B * grad_u_R * vectorn, dim = 1) - torch.sum((args.alpha_R * grad_u_in_R + Q) * vectorn, dim = 1), 2) * arclen)   
