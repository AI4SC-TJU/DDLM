...
def DirichletSolverPINN(args, traindata_bndry_G, dataloader_bndry_G, iter_num, sub_dom=1):
    ...
    logger.set_names(['epoch','loss_intrr','loss_bndry_D', 'loss_bndry_G', 'loss'])  
    fig = plt.figure()
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            loss_bndry_G = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_G) - torch.squeeze(g_1_select), 2))
