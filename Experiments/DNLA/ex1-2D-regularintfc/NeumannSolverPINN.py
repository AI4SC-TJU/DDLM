...
def NeumannSolverPINN(args, traindata_bndry_G, dataloader_bndry_G, model_left, iter_num, sub_dom=1):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            smppts_bndry_G, g_1_select = data_bndry_G
            ...
            u_NN_G_left = model_left(smppts_bndry_G)
            grad_u_R_left = torch.autograd.grad(u_NN_G_left,smppts_bndry_G,torch.ones_like(u_NN_G_left), retain_graph=True, create_graph=True, only_inputs=True)[0].detach()
             
            loss_bndry_G = torch.mean(torch.pow(grad_u_R[:, 0] - grad_u_R_left[:, 0], 2))   
            ...
            loss_epoch += loss_intrr_epoch  + args.beta * (loss_bndry_D_epoch + loss_bndry_G_epoch)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.005, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01, amsgrad=False)
    ...
    logger.set_names(['epoch','loss_intrr','loss_bndry_D', 'loss_bndry_N', 'loss'])  
    # train and test 
    ...
    for epoch in range(args.num_epochs):        
        ...
        logger.append([epoch, trainloss_intrr_epoch, trainloss_bndry_D_epoch, trainloss_bndry_G_epoch, trainloss_epoch])
