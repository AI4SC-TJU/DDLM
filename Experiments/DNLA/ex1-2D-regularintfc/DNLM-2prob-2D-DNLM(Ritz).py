...
from DirichletSolverRitz import *
...
parser.add_argument('-r', '--result', default='Results/1_2Prob-2Dcos/Compensent/Ritz/G1e_2-N2e4-baseline/simulation-test', type=str, metavar='PATH', help='path to save checkpoint')
...
while((ite_index < args.max_ite_num)):
    ...
    args.milestones=[600,800]
    model_left, error_L2_left, error_H1_left = DirichletSolverRitz(args, traindata_bndry_G, dataloader_bndry_G, SmpPts_Intfc, g_left, ite_index, 1)
    # update Robin boundary condition for right subproblem
    g_right_temp = model_left(SmpPts_Intfc)
    g_right = g_right_temp.detach()
