...
def NeumannSolverPINN(args, traindata_bndry_G, dataloader_bndry_G_1, dataloader_bndry_G_2, iter_num, u_cross, sub_dom=2):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G_1, data_bndry_G_2) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G_1), cycle(dataloader_bndry_G_2))):
            ...
            grad_u_NN_G_x = torch.autograd.grad(outputs=u_NN_bndry_G_1, inputs=smppts_bndry_G_1, grad_outputs=torch.ones_like(u_NN_bndry_G_1), retain_graph=True, create_graph=True, only_inputs=True)[0][:,0]
            grad_u_NN_G_y = torch.autograd.grad(outputs=u_NN_bndry_G_2, inputs=smppts_bndry_G_2, grad_outputs=torch.ones_like(u_NN_bndry_G_2), retain_graph=True, create_graph=True, only_inputs=True)[0][:,1]        
            ...
            loss_intrr = torch.mean((-args.alpha_B*laplace_u-f_smppts)**2)
            loss_bndry_D = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_D) - g_D_smppts, 2))

            loss_bndry_R =  torch.mean(( grad_u_NN_G_x- g_1_select.reshape(1,-1))**2)       
            loss_bndry_R += torch.mean(( grad_u_NN_G_y- g_2_select.reshape(1,-1))**2)
