...
from DirichletSolver4probhighcontrastRitz import DirichletSolverRitz
from CompensentedSolver4probhighcontrast import CompensentSolver

############################################################################################
## hyperparameter configuration
## ----------------------------- ##
## parser arguments
parser = argparse.ArgumentParser(description='DNLM(Ritz) method for Poisson problem with 4 high-contrast coefficient(ex5)')

# path for saving results
parser.add_argument('-r', '--result', default='Results/6_4Prob-2D-highcontrast-4NN/DNLM(Ritz)/G1e_2-N2e4-baseline/simulation-test', type=str, metavar='PATH', help='path to save checkpoint')
...
parser.add_argument('--mu', default=100, type=int, metavar='N', help='penalty coefficeint for mismatching of boundary data')
...
'''
g_1 = h_exact_1.detach()
g_2 = h_exact_2.detach()
g_3 = h_exact_3.detach()
g_4 = h_exact_4.detach()
'''
...
while((ite_index < args.max_ite_num)):
    ...
    args.milestones = [600, 1500]
    model_1, error_L2_1, error_H1_1 = DirichletSolverRitz(args, traindata_bndry_G_2, dataloader_bndry_G_1, dataloader_bndry_G_2, ite_index, u_cross , sub_dom=1)    
    
    u_cross = model_1(cross_point)
    u_cross = u_cross.detach()


    model_3, error_L2_3, error_H1_3 = DirichletSolverRitz(args, traindata_bndry_G_2, dataloader_bndry_G_3, dataloader_bndry_G_4, ite_index, u_cross, sub_dom=3)    
    
    u_cross = model_3(cross_point)
    u_cross = u_cross.detach()
    args.num_epochs = 5000
    args.milestones = [2000, 4000]
