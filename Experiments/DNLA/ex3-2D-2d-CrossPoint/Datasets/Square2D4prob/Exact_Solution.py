...
def f_Exact_Square2D(x, y):
    ...
    return 4 * np.pi * np.pi * torch.sin(2 * np.pi * x) * torch.sin(2 * np.pi * y) + 4 * np.pi * np.pi * torch.sin(2 * np.pi * x) * torch.sin(2 * np.pi * y)
...
def Gradu_x_Exact_Square2D(x, y):
	...
	return 2 * np.pi * torch.cos(2 * np.pi * x) * torch.sin(2 * np.pi * y)

# grad_y of exact solution	 
def Gradu_y_Exact_Square2D(x, y):

	return 2 * np.pi * torch.sin(2 * np.pi * x) * torch.cos(2 * np.pi * y)
