...
def SolverDirichletRitz(args, traindata_intfc, dataloader_intfc_1, dataloader_intfc_2, dataloader_intfc_3, dataloader_intfc_4, iter_num, u_cross, sub_dom = 'R'):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry, data_intfc_1, data_intfc_2, data_intfc_3, data_intfc_4) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_intfc_1), cycle(dataloader_intfc_2), cycle(dataloader_intfc_3), cycle(dataloader_intfc_4))):
            ...
            loss_bndry_D_epoch += loss_bndry.item() * smppts_bndry.size(0) / traindata_bndry.SmpPts_bndry.shape[0]
            loss_bndry_G_epoch += loss_intfc.item() * smppts_intfc_1.size(0) / traindata_intfc.SmpPts_intfc.shape[0]
            loss_epoch += loss_intrr_epoch + args.beta *(loss_bndry_G_epoch +  loss_bndry_D_epoch + loss_cross.item()) 
