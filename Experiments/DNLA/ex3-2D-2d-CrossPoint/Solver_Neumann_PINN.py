...
def SolverNeumannPINN(args, traindata_intfc, dataloader_intfc_1, dataloader_intfc_2, dataloader_intfc_3, dataloader_intfc_4, iter_num, sub_dom = 'R'):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry, data_intfc_1, data_intfc_2, data_intfc_3, data_intfc_4) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_intfc_1), cycle(dataloader_intfc_2), cycle(dataloader_intfc_3), cycle(dataloader_intfc_4))):
            ...
            loss_intfc =  torch.mean(( grad_u_1[:,0]- h_1_smppts.reshape(1,-1))**2)       
            loss_intfc += torch.mean(( grad_u_2[:,1]- h_2_smppts.reshape(1,-1))**2)
            loss_intfc += torch.mean(( grad_u_3[:,0]- h_3_smppts.reshape(1,-1))**2) 
            loss_intfc += torch.mean(( grad_u_4[:,1]- h_4_smppts.reshape(1,-1))**2)  
