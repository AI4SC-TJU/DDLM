...
## parser arguments
parser = argparse.ArgumentParser(description='DNLM(Ritz) method for Poisson problem with 4 sub-domains (ex3)')
...
while((ite_index < args.max_ite_num)):
    ...
    model_R, error_L2_R, error_H1_R = SolverDirichletRitz(args,  traindata_intfc_2, dataloader_intfc_1, dataloader_intfc_2, dataloader_intfc_3, dataloader_intfc_4, ite_index, u_cross,  sub_dom='R')
    u_cross = model_R(cross_point)
    u_cross = u_cross.detach()

    # Black subproblem-solving

    model_B, error_L2_B, error_H1_B = SolverNeumannDNLA(args, model_R, ite_index, sub_dom='B')
