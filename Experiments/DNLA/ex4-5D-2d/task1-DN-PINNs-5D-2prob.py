...
## ----------------------------- ##
testdata_left = Testdata(args.num_test_pts, 1, dim_prob )
testdata_right = Testdata(args.num_test_pts, 2, dim_prob)
...
# initial the variable u_left and u_right, which is used to store the solution of previous step
...
while((ite_index < args.max_ite_num)):
    ...
    # update Neumann boundary condition for right subproblem
    
    g_right_temp = model_left(SmpPts_Intfc)
    #g_right_temp = Exact_Solution.u_Exact_Square5D(SmpPts_Intfc, dim_prob).reshape(-1,1)
    ...
    traindata_intfc.h_smppts = g_right.detach()
    # right subproblem-solving

    model_right, error_L2_right, error_H1_right = SolverNeumannPINN(args, traindata_intfc, ite_index, 2)
