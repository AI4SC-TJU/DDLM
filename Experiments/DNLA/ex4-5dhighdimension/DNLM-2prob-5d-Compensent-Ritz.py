...
from DirichletSolverRitz5d import DirichletSolverRitz
from CompensentedSolver5d import CompensentSolver
...
parser.add_argument('-r', '--result', default='Results/4_2Prob-5D/DNLM(Ritz)/G1e_2-N2e4-baseline/simulation-test', type=str, metavar='PATH', help='path to save checkpoint')
...
class TraindataIntfc(Dataset):    
    def __init__(self, num_bndry_pts_G, dim_prob):         
        ...
        self.g_1_smppts = Exact_Solutionplus.u_Exact_Square10D(self.SmpPts_Bndry_G).reshape(-1,1)
    def __len__(self):
...
class Testdata(Dataset):    
    def __init__(self, num_test_pts, sub_dom): 
        ...
        self.Gradu_Exact_SmpPts = Exact_Solutionplus.Gradu_Exact_Square10D(self.SmpPts_Test)   
...
while((ite_index < args.max_ite_num)):
    ...
    model_left, error_L2_left, error_H1_left = DirichletSolverRitz(args, traindata_bndry_G, dataloader_bndry_G, ite_index, 1)
    #model_left.load_state_dict(torch.load(os.path.join(args.result, 'model_left.pt'), map_location="cuda"))
