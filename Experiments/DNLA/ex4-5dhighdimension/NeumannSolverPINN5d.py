...
from Models.FcNet.Resnet import ResNet
...
def NeumannSolverPINN(args, traindata_bndry_G, dataloader_bndry_G, iter_num, sub_dom=1):
    ...
    ##############################################################################################
    print(traindata_bndry_D.SmpPts_Bndry_D.size())    
    print(traindata_bndry_G.SmpPts_Bndry_G.size()) 
    print(traindata_intrr.SmpPts_Interior.size()) 
    ##############################################################################################
    print('*', '-' * 45, '*')
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G))):
            ...
            '''
            c = findidx(SmpPts_Intfc,smppts_bndry_G).cpu().numpy()[:len(smppts_bndry_G)]
            ...
            loss_interior = torch.mean((-laplace_u-f_smppts)**2)
    ...
    #schedular = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 100, eta_min=0, last_epoch=- 1, verbose=False)
    ...
    grad_u_Exact = grad_u_Exact.cpu().detach().numpy()
