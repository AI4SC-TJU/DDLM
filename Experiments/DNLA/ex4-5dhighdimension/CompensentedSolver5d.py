...
def CompensentSolver(args, model_left, iter_num, sub_dom=1):
    ...
    def train_epoch(epoch, model, optimizer, device):
        ...
        for i, (data_intrr_N, data_bndry_N, data_intrr_D, data_bndry_D) in enumerate(zip(dataloader_intrr_N, cycle(dataloader_bndry_N), cycle(dataloader_intrr_D), cycle(dataloader_bndry_D))):
            ...
            loss_intrr_N_1 = torch.mean(torch.pow(torch.squeeze(-laplace_u) - f_smppts_N, 2))
            loss_bndry_N = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_N) - g_smppts_N, 2))
            u_NN_left = model_left(smppts_intrr_D)
            #u_NN_left = Exact_Solutionplus.u_Exact_Square10D(smppts_intrr_D).reshape(-1,1)
            #u_NN_left = Exact_Solutionplus.u_Exact_Square10D(smppts_intrr_D)
            ...
            loss_minibatch = loss_intrr_N +  loss_intrr_N_1 - loss_intrr_D + args.beta * (loss_bndry_N + loss_bndry_D)
