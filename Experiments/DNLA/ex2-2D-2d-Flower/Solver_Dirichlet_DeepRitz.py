...
def SolverDirichletDeepRitz(args, traindata_intfc, iter_num, sub_dom=1):
    ...
    # create training and testing datasets
    traindata_intrr = TraindataInterior(args, sub_dom)
    ...
    plt.figure()
    plt.scatter(traindata_intrr.SmpPts_intrr[:,0], traindata_intrr.SmpPts_intrr[:,1], c = 'red' , label = 'interior points sub_dom1' )
    plt.scatter(traindata_bndry.SmpPts_bndry[:,0],  traindata_bndry.SmpPts_bndry[:,1], c = 'black', label = 'boundary of sub_dom1' )
    plt.scatter(traindata_intfc.SmpPts_intfc[:,0],  traindata_intfc.SmpPts_intfc[:,1], c = 'blue', label = 'interface' )
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry, data_intfc) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_intfc))):
            ...
            loss_intfc = torch.mean(torch.pow(torch.squeeze(u_NN_intfc) - torch.squeeze(h_smppts), 2) * arclen)   
