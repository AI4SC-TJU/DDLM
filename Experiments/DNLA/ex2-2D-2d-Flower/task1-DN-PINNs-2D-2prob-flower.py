...
## ----------------------------- ##
## parser arguments
parser = argparse.ArgumentParser(description='DN-PINNs method for Poisson problem with flower-shape interface (ex2)')
...
logger = helper.Logger(os.path.join(args.result, 'log.txt'), title='DN-PINNs-2Prob-2D-flower')
...
while((ite_index < args.max_ite_num)):
    ...
    errorL2_out, errorH1_out, model_out = SolverNeumannPINN(args, traindata_intfc, model_in, ite_index, 2)
