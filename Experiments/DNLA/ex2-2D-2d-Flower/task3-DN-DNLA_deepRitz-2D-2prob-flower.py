...
## ----------------------------- ##
## parser arguments
parser = argparse.ArgumentParser(description='DNLA (Ritz) method for Poisson problem with flower-shape interface (ex2)')
...
logger = helper.Logger(os.path.join(args.result, 'log.txt'), title='DNLA-Ritz-2Prob-2D-flower')
...
while((ite_index < args.max_ite_num)):
    ...
    errorL2_in, errorH1_in, model_in = SolverDirichletDeepRitz(args, traindata_intfc, ite_index, 1)
