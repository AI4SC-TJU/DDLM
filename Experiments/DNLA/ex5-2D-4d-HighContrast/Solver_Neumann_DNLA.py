...
def SolverNeumannDNLA(args, model_E_1, model_E_2, iter_num, sub_dom = '2'):
    ...
    traindata_intrr_E_1= TraindataInterior(args.num_intrr_pts, Extensionarea[0], args.c_1, args.c_2, dim_prob)
    traindata_bndry_E_1 = TraindataBoundaryDirichlet(args.num_bndry_pts, Extensionarea[0], args.c_1, args.c_2, dim_prob)
    # define dataloader 
    dataloader_intrr_E_1 = DataLoader(traindata_intrr_E_1, batch_size=batchsize_intrr_pts, shuffle=True, num_workers=0)
    dataloader_bndry_D_E_1 = DataLoader(traindata_bndry_E_1, batch_size=batchsize_bndry_pts, shuffle=True, num_workers=0)

    traindata_intrr_E_2= TraindataInterior(args.num_intrr_pts, Extensionarea[1], args.c_1, args.c_2, dim_prob)
    traindata_bndry_D_E_2 = TraindataBoundaryDirichlet(args.num_bndry_pts, Extensionarea[1], args.c_1, args.c_2, dim_prob)
    # define dataloader 
    dataloader_intrr_E_2 = DataLoader(traindata_intrr_E_1, batch_size=batchsize_intrr_pts, shuffle=True, num_workers=0)
    dataloader_bndry_D_E_2 = DataLoader(traindata_bndry_E_1, batch_size=batchsize_bndry_pts, shuffle=True, num_workers=0)
    ...
    plt.scatter(traindata_intrr_E_1.SmpPts_intrr[:,0], traindata_intrr_E_1.SmpPts_intrr[:,1], c = 'blue', label = 'interior points Extension area 1' )
    plt.scatter(traindata_intrr_E_2.SmpPts_intrr[:,0], traindata_intrr_E_2.SmpPts_intrr[:,1], c = 'grey', label = 'interior points Extension area 2' )
    plt.scatter(traindata_bndry.SmpPts_bndry[:,0],  traindata_bndry.SmpPts_bndry[:,1], c = 'green', label = 'boundary of right' )
    plt.scatter(traindata_bndry_E_1.SmpPts_bndry[:,0],  traindata_bndry_E_1.SmpPts_bndry[:,1], c = 'black', label = 'boundary of Extension area 1' )
    plt.scatter(traindata_bndry_D_E_2.SmpPts_bndry[:,0],  traindata_bndry_D_E_2.SmpPts_bndry[:,1], c = 'yellow', label = 'boundary of Extension area 2' )
    ...
    def train_epoch(epoch, model, optimizer, device):
        ...
        for i, (data_intrr, data_intrr_E_1, data_intrr_E_2, data_bndry, data_bndry_E_1, data_bndry_E_2) in enumerate(zip(dataloader_intrr, cycle(dataloader_intrr_E_1), cycle(dataloader_intrr_E_2), cycle(dataloader_bndry), cycle(dataloader_bndry_D_E_1), cycle(dataloader_bndry_D_E_2))):
            ...
            smppts_bndry_E_1, h_E_1_smppts = data_bndry_E_1
            smppts_bndry_E_2, h_E_2_smppts = data_bndry_E_2
            ...
            smppts_bndry_E_1 = smppts_bndry_E_1.to(device)
            smppts_bndry_E_2 = smppts_bndry_E_2.to(device)
            g_smppts = g_smppts.to(device)
            h_E_1_smppts = h_E_1_smppts.to(device)
            h_E_2_smppts = h_E_2_smppts.to(device)
            ...
            u_NN_bndry_E_1 = model(smppts_bndry_E_1)
            u_NN_bndry_E_2 = model(smppts_bndry_E_2)
            ...
            loss_intrr = torch.mean(0.5 * args.c_2 * torch.sum(torch.pow(grad_u_NN_intrr, 2), dim=1) - f_smppts * torch.squeeze(u_NN_intrr))
            loss_intrr += torch.mean((-args.c_2 * laplace_u-f_smppts.reshape(-1,1))**2)
            ...
            loss_intrr_E_1 = torch.mean(-f_smppts_E_1 * torch.squeeze(u_NN_intrr_E_1)) + args.c_1 * torch.mean( grad_u_E_1[:,0] * torch.squeeze(grad_u_NN_intrr_E_1[:,0]) + grad_u_E_1[:,1] * torch.squeeze(grad_u_NN_intrr_E_1[:,1]))
            loss_intrr_E_2 = torch.mean(-f_smppts_E_2 * torch.squeeze(u_NN_intrr_E_2)) + args.c_1 * torch.mean( grad_u_E_2[:,0] * torch.squeeze(grad_u_NN_intrr_E_2[:,0]) + grad_u_E_2[:,1] * torch.squeeze(grad_u_NN_intrr_E_2[:,1]))
            loss_bndry_E_1 = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_E_1) - h_E_1_smppts, 2))         
            loss_bndry_E_2 = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_E_2) - h_E_2_smppts, 2))
            ...
            loss_intrr_E_epoch += loss_intrr_E.item() * smppts_intrr_E_1.size(0) / traindata_intrr_E_1.SmpPts_intrr.shape[0]
            loss_bndry_E_epoch += loss_bndry_E.item() * smppts_bndry_E_1.size(0) / traindata_bndry_E_1.SmpPts_bndry.shape[0]
