...
def DirichletSolverRitz(args, traindata_bndry_G, dataloader_bndry_G_1, dataloader_bndry_G_2, dataloader_bndry_G_3, dataloader_bndry_G_4, iter_num, u_cross, sub_dom='R'):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G_1, data_bndry_G_2, data_bndry_G_3, data_bndry_G_4) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G_1), cycle(dataloader_bndry_G_2), cycle(dataloader_bndry_G_3), cycle(dataloader_bndry_G_4))):
            ...
            loss_intrr = torch.mean(0.5 * torch.sum(torch.pow(grad_u_NN_intrr, 2), 1) - torch.squeeze(f_smppts) * torch.squeeze(u_NN_intrr))
            ...
            loss_bndry_R += torch.mean(( u_NN_bndry_G_3- g_3_select.reshape(-1,1))**2) 
            loss_bndry_R += torch.mean(( u_NN_bndry_G_4- g_4_select.reshape(-1,1))**2)  
