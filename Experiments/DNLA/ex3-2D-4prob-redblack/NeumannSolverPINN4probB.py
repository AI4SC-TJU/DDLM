...
def NeumannSolverPINN(args, traindata_bndry_G, dataloader_bndry_G_1, dataloader_bndry_G_2, dataloader_bndry_G_3, dataloader_bndry_G_4, iter_num, u_cross, sub_dom=1):
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry_D, data_bndry_G_1, data_bndry_G_2, data_bndry_G_3, data_bndry_G_4) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry_D), cycle(dataloader_bndry_G_1), cycle(dataloader_bndry_G_2), cycle(dataloader_bndry_G_3), cycle(dataloader_bndry_G_4))):
            ...
            smppts_bndry_G_3, g_3_select = data_bndry_G_3
            smppts_bndry_G_4, g_4_select = data_bndry_G_4
            ...
            smppts_bndry_G_3 = smppts_bndry_G_3.to(device)
            smppts_bndry_G_4 = smppts_bndry_G_4.to(device)
            g_1_select = g_1_select.to(device)
            g_2_select = g_2_select.to(device)
            g_3_select = g_3_select.to(device)
            g_4_select = g_4_select.to(device)

            smppts_intrr.requires_grad = True

            smppts_bndry_G_1.requires_grad = True
            smppts_bndry_G_2.requires_grad = True
            smppts_bndry_G_3.requires_grad = True
            smppts_bndry_G_4.requires_grad = True
            ...
            u_NN_bndry_G_3 = model(smppts_bndry_G_3)
            u_NN_bndry_G_4 = model(smppts_bndry_G_4)
            cross_point = 0.5*torch.ones(100,2).to(device)
            u_NN_cross = model(cross_point)
            #grad_u_NN_cross = torch.autograd.grad(outputs=u_NN_cross, inputs=cross_point, grad_outputs=torch.ones_like(u_NN_cross), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            ...
            # get the predict Neumann boundary condition
            grad_u_1 = torch.autograd.grad(outputs=u_NN_bndry_G_1, inputs=smppts_bndry_G_1, grad_outputs=torch.ones_like(u_NN_bndry_G_1), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            grad_u_2 = torch.autograd.grad(outputs=u_NN_bndry_G_2, inputs=smppts_bndry_G_2, grad_outputs=torch.ones_like(u_NN_bndry_G_2), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            grad_u_3 = torch.autograd.grad(outputs=u_NN_bndry_G_3, inputs=smppts_bndry_G_3, grad_outputs=torch.ones_like(u_NN_bndry_G_3), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            grad_u_4 = torch.autograd.grad(outputs=u_NN_bndry_G_4, inputs=smppts_bndry_G_4, grad_outputs=torch.ones_like(u_NN_bndry_G_4), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            # calculate the loss of Neumann boundary condition
            loss_bndry_G =  torch.mean(( grad_u_1[:,0]- g_1_select.reshape(1,-1))**2)       
            loss_bndry_G += torch.mean(( grad_u_2[:,1]- g_2_select.reshape(1,-1))**2)
            loss_bndry_G += torch.mean(( grad_u_3[:,0]- g_3_select.reshape(1,-1))**2) 
            loss_bndry_G += torch.mean(( grad_u_4[:,1]- g_4_select.reshape(1,-1))**2)  
            loss_cross   = torch.mean(torch.square(u_cross - u_NN_cross))
            loss_minibatch = loss_intrr + args.beta *(loss_bndry_D  + loss_bndry_G)            
            ...
            loss_bndry_G_epoch += loss_bndry_G.item() * smppts_bndry_G_1.size(0) / traindata_bndry_G.SmpPts_Bndry_G.shape[0]
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.learning_rate, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01, amsgrad=False)
