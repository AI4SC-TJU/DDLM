...
def CompensentSolverB(args, model_R, iter_num, u_cross, sub_dom='B'):
    ...
    traindata_bndry_D_B = TraindataBoundaryDirichlet(args.num_bndry_pts_D, sub_prob, dim_prob)
    ...
    dataloader_bndry_D_B = DataLoader(traindata_bndry_D_B, batch_size=batchsize_bndry_pts_D, shuffle=True, num_workers=0)
    ...
    traindata_bndry_D_R = TraindataBoundaryDirichlet(args.num_bndry_pts_D, sub_prob, dim_prob)
        
    dataloader_intrr_R = DataLoader(traindata_intrr_R, batch_size=batchsize_intrr_pts, shuffle=True, num_workers=0)
    dataloader_bndry_D_R = DataLoader(traindata_bndry_D_R, batch_size=batchsize_bndry_pts_D, shuffle=True, num_workers=0)
    ##############################################################################################
    plt.figure()

    plt.scatter(traindata_intrr_R.SmpPts_Interior[:,0], traindata_intrr_R.SmpPts_Interior[:,1], c = 'red' , label = 'interior points red block' )
    plt.scatter(traindata_intrr_B.SmpPts_Interior[:,0], traindata_intrr_B.SmpPts_Interior[:,1], c = 'blue', label = 'interior points black block' )
    plt.scatter(traindata_bndry_D_R.SmpPts_Bndry_D[:,0],  traindata_bndry_D_R.SmpPts_Bndry_D[:,1], c = 'green', label = 'boundary of red block' )
    plt.scatter(traindata_bndry_D_B.SmpPts_Bndry_D[:,0],  traindata_bndry_D_B.SmpPts_Bndry_D[:,1], c = 'black', label = 'boundary of black block' )
    ...
    def train_epoch(epoch, model, optimizer, device):
        ...
        for i, (data_intrr_B, data_intrr_R, data_bndry_D_B, data_bndry_D_R) in enumerate(zip(dataloader_intrr_B, cycle(dataloader_intrr_R), cycle(dataloader_bndry_D_B), cycle(dataloader_bndry_D_R))):
           ...
           # zero parameter gradients and then compute NN prediction of gradient u(x) for Black problem
            ...
            #u_NN_intrr_R = Exact_Solution.u_Exact_Square2D(smppts_intrr_R[:, 0],smppts_intrr_R[:, 1]).reshape(-1,1)
            ...
            #loss_cross   = torch.mean(torch.square(u_cross - u_NN_cross)) + torch.mean(torch.square(grad_u_cross - grad_u_NN_cross))            
            ...
            loss_intrr_B_1 = torch.mean((-laplace_u-f_smppts_B.reshape(-1,1))**2)
            ...
            #u_R = Exact_Solution.u_Exact_Square2D(smppts_intrr_R[:,0],smppts_intrr_R[:,1]).reshape(-1,1)
            ...
            #loss_minibatch =  400 * loss_intrr_B_1 + loss_intrr_B + loss_intrr_R + args.beta * (loss_bndry_B + loss_bndry_R + loss_cross)
            loss_minibatch =  loss_intrr_B_1 + loss_intrr_B + loss_intrr_R + (args.beta + 10 * epoch) * (loss_bndry_B + loss_bndry_R)
            ...
            loss_intrr_B_epoch += loss_intrr_B_1.item() * smppts_intrr_B.size(0) / traindata_intrr_B.SmpPts_Interior.shape[0]
            loss_intrr_B_epoch += loss_intrr_B.item() * smppts_intrr_B.size(0) / traindata_intrr_B.SmpPts_Interior.shape[0]
            loss_bndry_B_epoch += loss_bndry_B.item() * smppts_bndry_D_B.size(0) / traindata_bndry_D_B.SmpPts_Bndry_D.shape[0]
            loss_intrr_R_epoch += loss_intrr_R.item() * smppts_intrr_R.size(0) / traindata_intrr_R.SmpPts_Interior.shape[0]
            loss_bndry_R_epoch += loss_bndry_R.item() * smppts_bndry_D_R.size(0) / traindata_bndry_D_R.SmpPts_Bndry_D.shape[0]
            loss_epoch += loss_intrr_B_epoch + args.beta * (loss_bndry_B_epoch + loss_bndry_R_epoch)  

        return loss_intrr_B_epoch, loss_bndry_B_epoch, loss_intrr_R_epoch, loss_bndry_R_epoch, loss_epoch
    ##############################################################################################
