...
def SmpPts_Boundary_Square2D(num_bndry_pts, sub_dom,dim_prob=2):
    ...
    if sub_dom==1:
        ...
        # boundary of domain (0,1/2) * (0,1), not including the interface {1/2} * (0,1)          
    ...
    if sub_dom==2:
        ...
        # boundary of domain (1/2,1) * (0,1), not including the interface {1/2} * (0,1)          
...
def SmpPts_Interface_Square2D(num_bndry_pts, dim_prob=2):    
    """ num_bndry_pts = total number of boundary sampling points at interface
        dim_prob  = dimension of sampling domain """
