...
def SolverNeumannDNLA(args, model_left, iter_num, sub_dom=1):
    ...
    ############################################################################################## 
    logger = helper.Logger(os.path.join(args.result, 'log-ite-%d-%s.txt'%(iter_num, sub_dom)), title='Solver_Neumann_DNLA')
    logger.set_names(['epoch', 'loss_N_Ritz','loss_N_Ritz_exact', 'loss_intrr_N_PINN', 'loss_intrr_D', 'loss_intrr_D_exact','loss_bndry_D'])  

    
    fig = plt.figure()
    plt.scatter(traindata_intrr_N.SmpPts_intrr[:,0], traindata_intrr_N.SmpPts_intrr[:,1], c = 'red', label = 'interior points' )
    plt.scatter(traindata_bndry_N.SmpPts_bndry[:,0], traindata_bndry_N.SmpPts_bndry[:,1], c = 'blue', label = 'Dirichlet boundary points (on the boundary)')
    ...
    def train_epoch(epoch, model, optimizer, device):
        ...
        loss_intrr_N_epoch, loss_intrr_D_epoch, loss_bndry_N_epoch, loss_bndry_D_epoch, loss_epoch  = 0, 0, 0, 0, 0
        ...
        for i, (data_intrr_N, data_bndry_N, data_intrr_D, data_bndry_D) in enumerate(zip(dataloader_intrr_N, cycle(dataloader_bndry_N), cycle(dataloader_intrr_D), cycle(dataloader_bndry_D))):
            ...
            model.zero_grad()
            grad_u_NN_intrr_D = torch.autograd.grad(outputs=u_NN_intrr_D, inputs=smppts_intrr_D, grad_outputs = torch.ones_like(u_NN_intrr_D), retain_graph=True, create_graph=True, only_inputs=True)[0]                

            # construct mini-batch loss function and then perform backward pass
            
            loss_intrr_N = torch.mean(0.5 * torch.sum(torch.pow(gradu_NN_intrr_N, 2), dim=1) - f_smppts_N * torch.squeeze(u_NN_intrr_N)) # \int_{\Omega_2} 0.5 * |\nabla u_2|^2 - f * u_2 dx
            loss_intrr_N_PINN =  torch.mean(torch.pow(-torch.squeeze(laplace_u_N) - torch.squeeze(f_smppts_N), 2)) # \int_{\Omega_2} \Vert u_2 - f \Vert^2 dx, the PINN loss on the Neumann problem, which is used to accelerate the optimization progress
            loss_bndry_N = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_N) - g_smppts_N, 2))
            u_NN_left = model_left(smppts_intrr_D) # this represent $u_1^{[k]}$
            u_exact_N = Exact_Solution.u_Exact_Square2D(smppts_intrr_N[:,0],smppts_intrr_N[:,1]) # this represent the exact solution on the Neumann subproblem
            
            grad_u_exact_N = torch.autograd.grad(outputs=u_exact_N, inputs=smppts_intrr_N, grad_outputs = torch.ones_like(u_exact_N), retain_graph=True, create_graph=True, only_inputs=True)[0]   # this gradient of the exact solution on the Neumann subproblem                                  
            loss_intrr_N_exact = torch.mean(0.5 * torch.sum(torch.pow(grad_u_exact_N, 2), dim=1) - f_smppts_N * torch.squeeze(u_exact_N)) # The Ritz energy for Neumann problem with exact solution, which is used for tuning the hyperparameters

            u_exact_D = Exact_Solution.u_Exact_Square2D(smppts_intrr_D[:,0],smppts_intrr_D[:,1]) # this represent the exact solution on the Dirichlet subproblem
            ...
            grad_u_exact_D = torch.autograd.grad(outputs=u_exact_D, inputs=smppts_intrr_D, grad_outputs = torch.ones_like(u_exact_D), retain_graph=True, create_graph=True, only_inputs=True)[0]                

            loss_intrr_D = torch.mean(f_smppts_D * torch.squeeze(u_NN_intrr_D)) - torch.mean( grad_u_left_intrr_D[:,0] * torch.squeeze(grad_u_NN_intrr_D[:,0]) + grad_u_left_intrr_D[:,1] * torch.squeeze(grad_u_NN_intrr_D[:,1]) ) # \int_{\Omega_1} u_1^{[k]} R_1\gamma_0(u_2)  - f R_1\gamma_0(u_2) dx
            loss_intrr_D_exact = torch.mean(f_smppts_D * torch.squeeze(u_exact_D)) - torch.mean( grad_u_exact_D[:,0] * torch.squeeze(grad_u_exact_D[:,0]) + grad_u_exact_D[:,1] * torch.squeeze(grad_u_exact_D[:,1]) ) # Substitude exact solution into this term, which is used for tuning the hyperparameters
            ...
            logger.append([epoch, loss_intrr_N.item(), loss_intrr_N_exact.item(), loss_intrr_N_PINN.item(), loss_intrr_D.item(), loss_intrr_D_exact.item(), loss_bndry_N.item()+loss_bndry_D.item()]) # check the distance between loss_intrr_N and loss_intrr_N_exact, together with the distance between loss_intrr_D and loss_intrr_D_exact, which will help to optimize the loss function
        ...
        return loss_intrr_epoch, loss_bndry_D_epoch, loss_epoch
    ##############################################################################################
