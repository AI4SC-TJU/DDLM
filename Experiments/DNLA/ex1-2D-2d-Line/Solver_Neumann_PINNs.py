...
def SolverNeumannPINN(args, traindata_intfc, model_left, iter_num, sub_dom=1):
    ...
    ##############################################################################################
    logger = helper.Logger(os.path.join(args.result, 'log-ite-%d-%s.txt'%(iter_num, sub_dom)), title='Solver_Dirichlet_PINNs')
    logger.set_names(['epoch','loss_intrr','loss_bndry', 'loss_intfc', 'loss'])  
    fig = plt.figure()
    plt.scatter(traindata_intrr.SmpPts_intrr[:,0], traindata_intrr.SmpPts_intrr[:,1], c = 'red', label = 'interior points' )
    plt.scatter(traindata_bndry.SmpPts_bndry[:,0], traindata_bndry.SmpPts_bndry[:,1], c = 'blue', label = 'Dirichlet boundary points (on the boundary)' )
    plt.scatter(traindata_intfc.SmpPts_intfc[:,0], traindata_intfc.SmpPts_intfc[:,1], c = 'green', label = 'Dirichlet boundary points (on the interface)' )
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry, data_intfc) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_intfc))):
            ...
            grad_u_NN_intfc = torch.autograd.grad(outputs=u_NN_intfc, inputs=smppts_intfc, grad_outputs=torch.ones_like(u_NN_intfc), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            
            u_left_intfc = model_left(smppts_intfc)
            grad_u_left_intfc = torch.autograd.grad(outputs=u_left_intfc, inputs=smppts_intfc, grad_outputs=torch.ones_like(u_left_intfc), retain_graph=True, create_graph=True, only_inputs=True)[0]        
            ...
            loss_intfc = torch.mean(torch.pow(torch.squeeze(grad_u_NN_intfc[:, 0]) - torch.squeeze(grad_u_left_intfc[:, 0]), 2))
            ...
            loss_epoch += loss_intrr_epoch  + args.beta * (loss_bndry_epoch + loss_intfc_epoch)
    ...
    for epoch in range(args.num_epochs):        
        ...
        logger.append([epoch, trainloss_intrr_epoch, trainloss_bndry_epoch, trainloss_intfc_epoch, trainloss_epoch])
