...
def SolverDirichletDeepRitz(args, traindata_intfc, iter_num, sub_dom=1):
    ...
    ##############################################################################################
    logger = helper.Logger(os.path.join(args.result, 'log-ite-%d-%s.txt'%(iter_num, sub_dom)), title='Sover_Dirichlet_DeepRitz')
    ...
    # plt.show()
    plt.savefig(os.path.join(args.result,'TrainSmpPts_D.png'))
    plt.close(fig)
    ...
    # plt.show()
    plt.savefig(os.path.join(args.result,'TestSmpPts_D.png'))
    plt.close(fig)
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry, data_intfc) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_intfc))):
            ...
            loss_intfc = torch.mean(torch.pow(torch.squeeze(u_NN_intfc) - torch.squeeze(h_smppts), 2))
    ...
    # helper.save_learncurve({'train_curve': train_loss, 'test_curve': test_loss}, curve=args.result)   
    logger.close()
