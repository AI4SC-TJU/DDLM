...
from Solver_Dirichlet_PINNs import SolverDirichletPINN
from Solver_Neumann_PINNs import SolverNeumannPINN
...
## ----------------------------- ##
test_smppts_left = Testdata(args.num_test_pts, 1).SmpPts_Test.to(device)
test_smppts_right = Testdata(args.num_test_pts, 2).SmpPts_Test.to(device)
...
traindata_intfc.g_1_smppts = g_left
# initial the variable u_left_temp and u_right_temp, which is used to store the consecutive solution
...
while((ite_index < args.max_ite_num)):
    ...
    model_left, error_L2_left, error_H1_left = SolverDirichletPINN(args, traindata_intfc, ite_index, 1)
    
    # right subproblem-solving

    model_right, error_L2_right, error_H1_right = SolverNeumannPINN(args, traindata_intfc, model_left, ite_index, 2)
