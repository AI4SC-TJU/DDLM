...
def SolverRobinRRLA(args, traindata_intfc, model_left, iter_num, sub_dom=1):
    ...
    def train_epoch(epoch, model, optimizer, device):
        ...
        model.train()
        loss_epoch, loss_intrr_N_epoch, loss_intrr_D_epoch, loss_bndry_D_epoch, loss_bndry_N_epoch, loss_bndry_D_epoch, loss_intfc_epoch = 0, 0, 0, 0, 0, 0, 0
        ...
        for i, (data_intrr_N, data_bndry_N, data_intrr_D, data_bndry_D, data_intfc) in enumerate(zip(dataloader_intrr_N, cycle(dataloader_bndry_N), cycle(dataloader_intrr_D), cycle(dataloader_bndry_D), cycle(dataloader_intfc))):
            ...
            u_left_D = model_left(smppts_intrr_D)
            u_left_intfc = model_left(smppts_intfc)
            grad_u_left_intrr_D = torch.autograd.grad(outputs=u_left_D, inputs=smppts_intrr_D, grad_outputs = torch.ones_like(u_left_D), retain_graph=True, create_graph=True, only_inputs=True)[0]                

            loss_intrr_D = torch.mean(f_smppts_D * torch.squeeze(u_NN_intrr_D)) - torch.mean( grad_u_left_intrr_D[:,0] * torch.squeeze(grad_u_NN_intrr_D[:,0]) + grad_u_left_intrr_D[:,1] * torch.squeeze(grad_u_NN_intrr_D[:,1]) )
            loss_bndry_D = torch.mean(torch.pow(torch.squeeze(u_NN_bndry_D) - g_smppts_D, 2))         

            loss_bndry_G = args.alpha_right * torch.mean(u_left_intfc * u_NN_intfc - 1/2 * u_NN_intfc * u_NN_intfc)
