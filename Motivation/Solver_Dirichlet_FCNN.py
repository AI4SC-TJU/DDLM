...
def SolverDirichletFCNN(args, traindata_intfc, iter_num, sub_dom=1):
    ...
    batchsize_bndry_pts = 3*args.num_bndry_pts // args.num_batches # Dirichlet boundaries (interface not included)
    ...
    class TraindataInterior(Dataset):    
        def __init__(self, num_intrr_pts, sub_dom, dim_prob): 
            ...
            self.smppts_intrr = Sample_Points.smppts_intrr_Square2D(num_intrr_pts, sub_dom, dim_prob)
    ...
    class TraindataBoundaryDirichlet(Dataset):    
        def __init__(self, num_bndry_pts_D, sub_dom, dim_prob):         
            ...
            self.smppts_bndry = Sample_Points.SmpPts_Boundary_Square2D(num_bndry_pts_D, sub_dom, dim_prob)
    ...
    traindata_bndry = TraindataBoundaryDirichlet(args.num_bndry_pts_D, sub_dom, dim_prob)
    ...
    def train_epoch(epoch, model, optimizer, device, sub_dom):
        ...
        for i, (data_intrr, data_bndry, data_intfc) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_intfc))):
            ...
            loss_intfc = torch.mean((u_NN_intfc - h_smppts.reshape(-1,1))**2)       
